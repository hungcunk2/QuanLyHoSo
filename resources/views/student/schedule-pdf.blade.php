...
<body>
@php
    $heading = $range === 'month' ? 'Lịch học theo tháng' : 'Lịch học theo tuần';
@endphp
<div class="header">
    <div>
        <div class="title">{{ $heading }}</div>
        <div class="meta muted">
            Sinh viên: <strong>{{ $student->ho_ten ?? $student->mssv ?? '—' }}</strong>
            @if(!empty($student->mssv))
                · MSSV: <strong>{{ $student->mssv }}</strong>
            @endif
        </div>
        <div class="meta muted">
            @if($range === 'month')
