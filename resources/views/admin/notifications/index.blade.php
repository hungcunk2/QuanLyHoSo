                ...
                <tbody>
                    @forelse($items as $it)
                        <tr>
                            ...
                            <td class="text-end">
                                <a class="btn btn-outline-primary btn-sm" href="{{ route('admin.notifications.edit', $it) }}">Sửa</a>
                                <form method="POST" action="{{ route('admin.notifications.destroy', $it) }}" class="d-inline" onsubmit="return confirm('Xóa thông báo này?')">
                                    @csrf
                                    @method('DELETE')
